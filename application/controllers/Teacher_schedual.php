...
defined('BASEPATH') OR exit('No direct script access allowed');
          ...
          class Teacher_schedual extends CI_Controller {
		  public function __construct() {
		  parent::__construct();
		  error_reporting(E_ALL ^ (E_NOTICE | E_WARNING));
		  $this->load->library('session');
		  $this->load->helper('url');
		  $this->load->model('Teacher_schedual_model');
	...
	public function teacher_schedual_view()
	{
		...
		$data['teacher_list'] = $this->Teacher_schedual_model->teacher_list();
        $this->load->view('teachers_schedual',$data);
	...
	public function insert_teacher_schedual()
	    {
		  $gt = new GoogleTranslate();
		   ...
		   /**/
		   $result = $this->Teacher_schedual_model->insert_teacher_schedual($data);
		  
		   //print_r($result);die();
			if($result == '1')
			{
			   $this->load->helper('url');
			   $this->session->set_flashdata('message','Successfully Added Teacher Schedual');
	...
	function fetch_data()
	{
		...
		$config = array();
		$config['base_url'] = '#';
		$config['total_rows'] = $this->Teacher_schedual_model->count_all($teacher_name,$schedual_day);
		$config['per_page'] = 8;
		...
		'admissions_list'   => $this->Teacher_schedual_model->fetch_data($config["per_page"], $start, $teacher_name,$schedual_day)
		);
		//print_r($output);die();
	...
	function delete_schedual($id)
	{
	  ...
	  if($result == 'true')
		{
			$this->load->helper('url');
			$this->session->set_flashdata('message','Teacher Schedual Successfully Deleted ');
			redirect('teacher_schedual/teacher_schedual_view');
		} else {
			$this->load->helper('url');
			redirect('teacher_schedual/teacher_schedual_view');
