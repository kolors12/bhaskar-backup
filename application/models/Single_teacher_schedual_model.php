...
defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Single_teacher_schedual_model extends CI_Model {
	   ...
	   public function get_subject()
		...
		{
		    $query=$this->db->where(array('role_in_school' =>'Teacher','staff_id' =>$id))->get('staff');
		...
		function insert_single_teacher_schedual($data){
		...
		function make_query($teacher_name,$schedual_day)
		{
			...
			WHERE ts.status IN (0,1) AND ts.teacher_name = '$var'  
			";
		...
		function fetch_data($limit, $start, $teacher_name,$schedual_day)
		{
			...
			if($data->num_rows() > 0)
			{
				...
				foreach($data->result_array() as $row)
				{
					...
					<tr>
						<td>'.$i.'</td>
						<td><strong>'. $f_name.' '. $l_name.'</strong></td>
